...
const ProductItemButtons = ({productData, isFavorite, isInStock, handleClickFavorite, classNames}: ProductItemButtonsProps): ReactElement => {
  ...
  return (
    <Box className={clsx('hidden flex-col items-center justify-between gap-3 sm:flex sm:items-end', classNames)}>
      ...
      {isInStock && <AddToWishListButton isFavorite={isFavorite} handleClickFavorite={handleClickFavorite} classNames="sm:flex hidden w-56 h-12" />}
    </Box>
  );
};
