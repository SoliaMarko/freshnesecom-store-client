...
interface ProductsFooterProps {
  productsMetadata: ProductsMetadataType;
  currentPage: number;
  handlePageChange: (page: number, action: PaginationButtonAction) => void;
}

const ProductsFooter = ({productsMetadata, currentPage, handlePageChange}: ProductsFooterProps): ReactElement => {
  const {pagesCount, itemsCount} = productsMetadata;
  ...
  return (
    <Box className="px:2 flex flex-col items-center justify-between gap-8 py-4 md:flex-row md:px-2">
      <Box className="flex flex-row items-center justify-between md:hidden">
        ...
        <AmountBlock value={itemsCount} label="products" classNames="order-3" />
      </Box>
      ...
      <AmountBlock value={itemsCount} label="products" classNames="hidden md:flex md:order-3" />
